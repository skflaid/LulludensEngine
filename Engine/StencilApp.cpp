...
void StencilApp::Draw(const GameTimer& gt)
{
	...
	CD3DX12_RESOURCE_BARRIER RB = CD3DX12_RESOURCE_BARRIER::Transition(CurrentBackBuffer(),
    ...
    // Indicate a state transition on the resource usage.
	CD3DX12_RESOURCE_BARRIER RB2 = CD3DX12_RESOURCE_BARRIER::Transition(CurrentBackBuffer(),
...
void StencilApp::UpdateMainPassCB(const GameTimer& gt)
{
	...
	XMVECTOR MDView = XMMatrixDeterminant(view);
	XMVECTOR MDProj = XMMatrixDeterminant(proj);
	XMVECTOR MDVP = XMMatrixDeterminant(viewProj);

	XMMATRIX invView = XMMatrixInverse(&MDView, view);
	XMMATRIX invProj = XMMatrixInverse(&MDProj, proj);
	XMMATRIX invViewProj = XMMatrixInverse(&MDVP, viewProj);
